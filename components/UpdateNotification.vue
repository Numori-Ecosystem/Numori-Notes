...
v-if="visible"
      ...
      <UiButton
variant="ghost" color="gray" icon-only class="absolute top-2 right-2" title="Dismiss"
        @click="$emit('dismiss')">
      ...
      <template v-if="!isNative">
        ...
        <UiButton variant="solid" color="primary" size="xs" class="flex-shrink-0" @click="$emit('apply')">
          Reload
        </UiButton>
      </template>
      ...
      <template v-else>
        ...
        <UiButton variant="solid" color="primary" block @click="$emit('apply')">
          Update
        </UiButton>
      </template>
    </div>
